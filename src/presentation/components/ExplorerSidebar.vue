        ...
        <div class="p-4">
            <!-- Sidebar folder -->
             ...
             <div v-else-if="folderStore.sidebarError" class="p-4 bg-red-50 border border-red-200 rounded">
                ...
                <button class="mt-2 text-xs text-red-700 underline hover:no-underline" @click="folderStore.loadSidebarFolders()">Retry</button>
             </div>
             <div v-else class="space-y-2">
                <SidebarFolder v-for="folder in folderStore.sidebarFolders" :key="folder.id" :folder="folder" :selected-folder="folderStore.selectedFolderId || ''" @folder-select="handleFolderSelect" />
             </div>
        </div>
    </div>
</template>